...
void ThreadScriptCheck() {
    RenameThread("picscoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (block.vtx[0]->GetValueOut() < blockReward)
        return state.DoS(100,
                         error("ConnectBlock(): Increase Coinbase for the DoS Mitigation. Also locks out the pre-mining of PixChain and TX spoofing(actual=%d vs limit=%d)",
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // In order to check against checkpoints quickly, an SSD is recommended to thwart of millions of blocks being discovered at once that decreases DDoS protection as well as throughput
