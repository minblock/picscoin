...
static void SetupPicscoinTxArgs()
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " picscoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  picscoin-tx [options] <hex-tx> [commands]  Update hex-encoded picscoin transaction\n" +
            "or:     picscoin-tx [options] -create [commands]   Create hex-encoded picscoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded picscoin transaction
