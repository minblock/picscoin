This directory contains the PicscoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Picscoin UI.
...
ClientModel is used by the main application `picscoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/picscoin/picscoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a picscoin:... link from a browser).
...
* UI elements like PicscoinAmountField, which inherit from QWidget.
* `picscoinstrings.cpp`: automatically generated
* `picscoinunits.(h/cpp)`: BTC / mBTC / etc handling
* `callback.h`
...
* don't change `local/picscoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "picscoin-qt" under "Run", which is where you can also add command line arguments)
