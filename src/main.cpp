...
uint256 hashGenesisBlock("0x2daeb8c3efaf9351643b51c5336818fba839dcc6d79c4585e341dab2d8776838");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Picscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Picscoin Signed Message:\n";
...
{
    // Picscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Picscoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Picscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Picscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Picscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1cef4a5283b4e4848cb85ad719312e50f7b63eebbce6627b53db498196fe302e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044e0d4bc823e20e14d66396a64960c993585400c53f1e6decb273f249bfeba0e71f140ffa7316f2cdaaae574e7d72620538c3e7791ae9861dfe84dd2955fc85e8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1545183172;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3633462;
...
unsigned char pchMessageStart[4] = { 0x4c, 0x65, 0x65, 0x74 }; // Picscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PicscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PicscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PicscoinMiner(CWallet *pwallet)
{
    printf("PicscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("picscoin-miner");
    ...
    try { loop {
        ...
        printf("Running PicscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PicscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PicscoinMiner, pwallet));
}
