Picscoin Core version 0.17.1 is now available from:

  <https://download.picsco.in/picscoin-0.17.1/>
...
When picscoin is configured with the `-deprecatedrpc=accounts` setting, specifying
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `picscoin.conf` file, not inside included files or from
  ...
  as picscoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the picscoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=picscoin
    test.uacomment=picscoin-testnet
...
be used in V0.17 by starting picscoind with the '-deprecatedrpc=accounts'
...
Overall, the construction of a fully signed Picscoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Picscoin transaction (in network format)
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When picscoin is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If picscoin is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart picscoind with
...
- When running picscoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If picscoind
...
- #13264 `2a7c53b` Sever unit (GreatSock)
