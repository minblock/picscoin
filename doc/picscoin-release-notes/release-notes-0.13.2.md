Picscoin Core version 0.13.2 is now available from:

  <https://download.picscoins.org/picscoin-0.13.2.1/>
...
Previous versions of Picscoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Picscoin Core was to raise the default minimum
relay fee.

Picscoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
This allows running picscoind without having to do any manual configuration.
...
Picscoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Picscoin Core will relay transactions with
...
entirely in Picscoin Core 0.13, and it is at this time undecided whether the
...
Picscoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Picscoin Core automatically creates a hidden service to listen on, without
manual configuration. Picscoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Picscoin Core is listening, and
...
Picscoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Picscoin
Core to use floating fees. By default, Picscoin Core will use floating
fees.
...
At all times, Picscoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) PIC.
...
`picscoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 21336 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 21336 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /picscoinrpc>
        ProxyPass http://127.0.0.1:1336/
        ProxyPassReverse http://127.0.0.1:1336/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass picscoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< picscoinrpc:<password>
    </Location>
...
Restarting picscoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in picscoin.conf
...
picscoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/picscoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Picscoin Core code base has
...
- `picscoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `picscoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Picscoin scripting system that in combination with
...
Picscoin transactions currently may specify a locktime indicating when
...
The two phases are distinct in the Picscoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Picscoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Picscoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Picscoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of picscoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of picscoins being spent is exactly
...
- **Increased security for multisig:** Picscoin addresses (both P2PKH addresses
...
- **More efficient almost-full-node security** Sevtokie Nagamokie's original
  ...
  note that Picscoin Core does not provide an option to use this capability as
...
segwit's start date of 1 January 2017 miners can update the Picscoin
client to Picscoin Core 0.13.2 to signal for segwit support. When a
...
Since Picscoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `picscoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `picscoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
