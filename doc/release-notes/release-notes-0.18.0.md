Picscoin Core version 0.18.0 is now available from:

  <https://picscoins.org/bin/picscoin-core-0.18.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://picscoins.org/en/list/announcements/join/>
...
`/Applications/Picscoin-Qt` (on Mac) or `picscoind`/`picscoin-qt` (on
Linux).
...
Picscoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Picscoin Core on unsupported systems.
...
10.10. Additionally, Picscoin Core does not yet change appearance when
...
`contrib/init/picscoind.service`, it has been changed to use
`/var/lib/picscoind` as the data directory instead of
`~picscoin/.picscoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/picscoind` will
exist has enough space (check using `df -h /var/lib/picscoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/picscoin/picscoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:1336:1336` (this is an extra `:1336` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://picscoins.org/en/contact/) the Picscoin Core
...
The systemd init file (`contrib/init/picscoind.service`) has been changed
to use `/var/lib/picscoind` as the data directory instead of
`~picscoin/.picscoin`. This change makes Picscoin Core more consistent with
...
`datadir` option in `/etc/picscoin/picscoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/picscoin/picscoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/picscoin/picscoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/picscoin/picscoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/picscoin/picscoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/picscoin/picscoin/blob/master/doc/picscoin-conf.md)
  about the `picscoin.conf` file describes how to use it to configure
  Picscoin Core.

- A new document introduces Picscoin Core's BIP174 [Partially-Signed
  Picscoin Transactions
  (PSBT)](https://github.com/picscoin/picscoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/picscoin/picscoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Picscoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Picscoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/picscoin/picscoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/picscoin/picscoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Picscoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Picscoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/picscoin/picscoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/picscoin/picscoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart picscoind with the `-deprecatedrpc=generate`
...
- A new `picscoin-wallet` tool is now distributed alongside Picscoin
...
This section describes planned changes to Picscoin Core that may affect
other Picscoin software and services.

- Since version 0.16.0, Picscoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Picscoin Core 0.20 (expected about
  a year after 0.18), Picscoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Picscoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Picscoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Picscoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Picscoin Core
...
- A sub-project of Picscoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Picscoin Core.  See their [project
  page](https://github.com/picscoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Picscoin Core's own implementation, although entropy
  gathered by Picscoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Picscoin Core a
...
- On macOS, Picscoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `picscoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libpicscoin_server.a` (jnewbery)
...
- #13926 [Tools] picscoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like picscoind (MarcoFalke)
...
- #14521 qt, docs: Fix `picscoin-qt -version` output formatting (hebasto)
...
- #11625 Add PicscoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libpicscoin_server` (laanwj)
...
- #15154 Configure: picscoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/picscoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Picscoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libpicscoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in picscoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Picscoin Core (Christewart)
...
- #12246 Bugfix: Only run picscoin-tx tests when picscoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_picscoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_picscoin` (MarcoFalke)
...
- #14186 picscoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_picscoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update picscoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/picscoin-conf.md (hebasto)
...
- #15354 Add missing `picscoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct picscoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support picscoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/picscoin/).
