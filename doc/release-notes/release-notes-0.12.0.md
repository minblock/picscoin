Picscoin Core version 0.12.0 is now available from:

  <https://picscoin.org/bin/picscoin-core-0.12.0/>
...
backwards-compatible with pre-0.10 versions of Picscoin Core or other software:
...
pre-0.12 versions of Picscoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Picscoin Core version 0.11 or
earlier.
...
Picscoin Core 0.12 will have a strict maximum size on the mempool. The
...
Picscoin Core 0.12 also introduces new default policy limits on the length and
...
Picscoin Core 0.12 nodes. Picscoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/picscoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Picscoin Core 0.12 does not yet have support for
...
notes of v0.11.0](https://github.com/picscoin/picscoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).
...
Then, to tunnel a SSL connection on 21336 to a RPC server bound on localhost on port 11336 do:

    stunnel -d 21336 -r 127.0.0.1:11336 -p stunnel.pem -P ''
...
- #6214 `87406aa` [OSX] revert renaming of Picscoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new picscoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include picscoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix picscoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of picscoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [picscoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libpicscoinconsensus (Braydon Fuller)
