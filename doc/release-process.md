...
By default, Gitian will fetch source files as needed. To cache them ahead of time, make sure you have checked out the tag you want to build in picscoin, then:
...
NOTE: Offline builds must use the --url flag to ensure Gitian fetches only from local URLs. For example:
    ...
    pushd ./gitian-builder
    ./bin/gbuild --url picscoin=/path/to/picscoin,signature=/path/to/sigs {rest of arguments}
    popd
...
Build output expected:
  ...
  1. source tarball (`picscoin-${VERSION}.tar.gz`)
...
Add other gitian builders keys to your gpg keyring, and/or refresh keys: See `../picscoin/contrib/gitian-keys/README.md`.
...
Codesigner only: Sign the macOS binary:
    ...
    transfer picscoin-osx-unsigned.tar.gz to macOS for signing
    tar xf picscoin-osx-unsigned.tar.gz
...
Codesigner only: Sign the windows binaries:
    ...
    tar xf picscoin-win-unsigned.tar.gz
...
space *do not upload these to the picsco.in server, nor put them in the torrent*.
...
- Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to the picsco.in server.

- Update picsco.in version
...
- Announce the release:
  ...
  - picscoin-dev mailing list

  - blog.picsco.in blog post
