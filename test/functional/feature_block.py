...
class FullBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Create a transaction that spends one sevtokie to the p2sh_script, the rest to OP_TRUE
        ...
        # Until block is full, add tx's with 1 sevtokie to p2sh_script, the rest to OP_TRUE
        tx_new = None
        tx_last = tx
        ...
        #                                                                                    \-> b68 (20)
        #
        # b68 - coinbase with an extra 10 sevtokies,
        #       creates a tx that has 9 sevtokies from out[20] go to fees
        #       this fails because the coinbase is trying to claim 1 sevtokie too much in fees
        #
        # b69 - coinbase with extra 10 sevtokies, and a tx that gives a 10 sevtokie fee
        #       this succeeds
        #
    ...
    def next_block(self, number, spend=None, additional_coinbase_value=0, script=CScript([OP_TRUE]), solve=True, *, version=1, version_override=False):
        ...
        if spend is None:
            ...
            coinbase.vout[0].nValue += spend.vout[0].nValue - 1  # all but one sevtokie to fees
            coinbase.rehash()
            block = create_block(base_block_hash, coinbase, block_time, version=blockversion)
            tx = self.create_tx(spend, 0, 1, script)  # spend 1 sevtokie
            self.sign_tx(tx, spend)
